module "s3_bucket" {
  ...
  bucket           = each.value.bucketNamespace == "account-regional" ? "${each.key}-${data.aws_caller_identity.caller_identity.account_id}-${local.config.region}-an" : each.key
  bucket_namespace = each.value.bucketNamespace
  force_destroy    = true
  cors_rule = each.value.presignedUrl == true ? [
    {
      allowed_headers = ["*"]
  ...
  object_ownership = each.value.websiteBucket == true ? "BucketOwnerEnforced" : "ObjectWriter"
